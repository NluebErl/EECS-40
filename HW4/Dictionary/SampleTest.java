...
// Test for FIND button
        myDictionary = new Dictionary();// "No Word Matched."
        myDictionary.TextNewWord.setText("NIKO");
        myDictionary.FINDButton.fire();
...
// Valid FIND Test 2
        ...
        myDictionary.TextArea.setText("Least Fortunate entry fragger");
        myDictionary.ADDButton.fire();
        myDictionary.CLEARButton.fire();myDictionary.TextNewWord.setText("nIKO");
        myDictionary.TextArea.setText("So-so Fortunate entry fragger");
        myDictionary.ADDButton.fire();
        myDictionary.CLEARButton.fire();
...
// freq of "nIKO" is 5
        ...
        myDictionary.CLEARButton.fire();
        if(myDictionary.searchHistoryList.getItems().get(0).equals("nIKO")&&
                myDictionary.searchHistoryList.getItems().get(1).equals("nIko")&&
                myDictionary.searchHistoryList.getItems().get(2).equals("niko")){
...
// test for WordNotFoundError
        myDictionary = new Dictionary();
        myDictionary.TextNewWord.setText("NIKO");try {
            myDictionary.REMOVEButton.fire();
...
// Test for MODIFY Button
        ...
        myDictionary.TextNewWord.setText("NIKO");
        myDictionary.MODIFYButton.fire();
        myDictionary.CLEARButton.fire();
        myDictionary.TextNewWord.setText("NIKO");
        myDictionary.FINDButton.fire();
        if (myDictionary.TextFreqWord1.getText().equals("NIKO")) {
...
// Test for IMPORT and EXPORT Button
        myDictionary = new Dictionary();
// Try multiple possible paths for input.txt
        String[] possibleInputPaths = {
                "src/Dictionary/input.txt", // From project root "input.txt", // Same directory as class files
                "Dictionary/src/Dictionary/input.txt", // From parent directory
                "../input.txt" // One level up
        };
        String inputPath = null;
        System.out.println("Working Directory: " + System.getProperty("user.dir"));
        for (String path : possibleInputPaths) {
            if (new java.io.File(path).exists()) {
                inputPath = path;
                System.out.println("Found input.txt at: " + path);
                break;
            }
        }
        if (inputPath == null) {
            System.out.println("ERROR: Could not find input.txt. Skipping import/export test.");
            return;
        }
        myDictionary.TextFilePath.setText(inputPath);
        myDictionary.IMPORTButton.fire();
        for (int numi = 0; numi < 3; numi++) {
// freq of "niko" is 3
            myDictionary.TextNewWord.setText("niko");
            myDictionary.FINDButton.fire();
...
// Use the same directory as input.txt for output.txt
        String outputPath = inputPath.replace("input.txt", "output.txt");
        myDictionary.TextFilePath.setText(outputPath);
        myDictionary.EXPORTButton.fire();
